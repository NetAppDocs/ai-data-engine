...
* Data scientist: Uses retrieval endpoints to power GenAI and RAG workflows, enabling models to “talk to their data” and provide context-aware responses.

=== Classifiers

Classifiers are tools (built-in or custom) that scan and tag files for specific types of sensitive data (e.g., PII, financial, healthcare) or categorize documents by type (e.g., legal, HR, sales).
