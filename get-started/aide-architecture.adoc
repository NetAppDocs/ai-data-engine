...
* Centralized metadata cataloging for all data sources, including volumes and buckets stored locally on the AFX cluster and those synchronized from remote ONTAP clusters.
* Automated metadata extraction and catalog population as new data is ingested or changed.
* REST API access for querying metadata, enabling data practitioners and storage admins to collaboratively discover, classify, and understand data.
...
* Automated, policy-driven synchronization of data from remote ONTAP clusters (separate ONTAP systems) or local ONTAP clusters (the AFX cluster in your AIDE deployment) using SnapMirror. Data from remote clusters is copied to the local AFX cluster, where it becomes available for AIDE processing.
...
.License and access
Data Curator functionality is provided as a separate supplemental software update.
