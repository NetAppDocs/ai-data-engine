...
== Configure external role mapping using the ONTAP CLI

External role mapping is needed to map the roles defined in your IdP server to the corresponding ONTAP roles. This allows ONTAP to enforce role-based access control (RBAC) for users authenticating through OIDC.

.About this task

This task maps the data engineer and data scientist to the corresponding ONTAP roles. You should have already mapped the storage administrator role to the ONTAP `admin` role during the basic OIDC configuration in System Manager.

.Steps

. Using SSH, sign in to the ONTAP CLI using an account with administrative privileges.

. Configure role mapping for the data scientist role.
+
[source,bash]
----
