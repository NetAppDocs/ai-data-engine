...
Connect the DX50 data compute nodes to redundant host network switches to enable client data access and AI workload processing. Use ports e4b and e5b on each node to establish dual connections for high availability and load balancing.
...
Connect the DX50 data compute nodes to the same cluster switches used by the AFX 1K controller nodes to enable communication within the storage cluster. Use ports e4a and e5a on each node for redundant cluster network connectivity. Connect to non-ISL ports on the cluster switches to ensure proper network segmentation. 
