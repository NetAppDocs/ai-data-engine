...
|`\https://582244788873.dkr.ecr.us-east-1.amazonaws.com`
|NetApp container registry (AIDE container images and Helm charts)
|Both
...
These sizes are recommended baseline configurations optimized for a 3 to 4 day initial scan window, not hard limits. Refer to <<flexible-sizing-lite,Flexible sizing guidelines>> for what drives these numbers and how to scale beyond them.
...
<<lite-vm-sizes,Small, Medium, and Large configurations>> are recommended baselines, not hard limits enforced by the software. They are designed to achieve optimal initial catalog completion, typically 3 to 4 days for a full scan.
...
These sizes are recommended baseline cluster configurations optimized for a ~3 day initial scan window, not hard capacity limits. AIDE Enterprise supports environments beyond 6 billion files through horizontal scale-out. Refer to <<flexible-sizing-enterprise,Flexible sizing guidelines>> for details.
