...
<<lite-vm-sizes,Small, Medium, and Large configurations>> are recommended baselines for a 3 to 4 day initial scan window, not hard limits enforced by the software.
...
+
* *Extended initial catalog duration*: Reduced compute lowers objects scanned per day. For example, a deployment targeting 3 billion objects but provisioned at a smaller size can take significantly longer than the 3 to 4 day target.
