...
You'll perform all data collection tasks in AIDE Console.

.Before you begin

* You need _data engineer_ or _data scientist_ privileges in AIDE Console (`\https://<cluster_management_ip>/console`).
...
* The AIDE software license is installed and inferencing features are enabled.
...
.Result
The collection definition and its embeddings are removed from AIDE. Applications attempting to query the former retrieval endpoint will fail after the collection is removed.
