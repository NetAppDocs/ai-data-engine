...
These limitations apply to the Metadata Engine and management workflows in AIDE 9.18.1.
...
+
Only ONTAP volumes (local or remote) are supported as data sources for workspaces and metadata cataloging. ONTAP S3 buckets and StorageGRID objects are not supported in this release.
...
+
Reverting DCN cluster software to an earlier version is not supported. Only upgrades to later versions are allowed.

* **ONTAP upgrade restrictions**
+
AIDE clusters cannot be reverted to earlier ONTAP versions.
