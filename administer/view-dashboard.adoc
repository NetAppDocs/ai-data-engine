...
[.lead]
You can sign in to System Manager to access the AIDE dashboard and display the cluster status. This is a good first step before beginning your AIDE administrative tasks or if you suspect an operational issue.
...
* An ONTAP administrator account. Data engineer and data scientist roles cannot access System Manager.

== Monitor AIDE health and capacity from the dashboard

. Connect to System Manager using the cluster management address:
+
`\https://$FQDN_OR_IP/`
...
. In the navigation pane, select *Data engine* and then *Activity*.
...
* Use workspace views and the Activity page to investigate any workspaces in *Error* or long‑running *Processing* state.
