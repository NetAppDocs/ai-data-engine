...
[.lead]
You need to replace a data compute node (DCN) in your cluster if it stops functioning or needs to be swapped due to hardware failure, upgrade, or maintenance. This ensures the AIDE cluster remains healthy and operational. The procedure can be performed without disrupting ongoing services.

[TIP]
[.red]*DP: Need to review the following page:* link:../administer/dcn-add-node.html[Expand your compute cluster]
...
There are several several restrictions to be aware of when replacing a node in your AIDE cluster:
...
. Delete the node from the cluster using the following command:
+
