...
[.lead]
You can add Data Compute Nodes (DCNs) as part of creating a new AI Data Engine (AIDE) cluster or expanding an existing cluster. In both cases, the workflow consists of discovering and configuring the nodes using ONTAP System Manager.
...
. Confirm there are nodes to add and select *View details* to display the list.
+
...
. Select *Add* above the list of nodes at the bottom of the data compute page.
...
. Wait for the operation to complete. System Manager will perform the following actions:
+
...
. After completion, select *Cluster* and *Overview*; under the *Data compute* tab confirm the new DCNs appear as part of the cluster and are *Healthy*.
